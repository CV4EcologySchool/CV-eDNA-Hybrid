# -*- coding: utf-8 -*-
"""
Created on Fri Sep 22 22:06:20 2023
...
from dataset_concat import CTDataset
from concat_direct import CustomResNet18, ConcatenateModel
...
def load_model(cfg):
    ...
    concatenated_model = ConcatenateModel(resnet_model, num_classes = cfg['num_classes'], num_col = cfg['num_col'])
    ...
    return concatenated_model, start_epoch
...
def train(cfg, dataLoader, model, optimizer):
    ...
    for idx, (X, imgs, labels) in enumerate(dataLoader):       # see the last line of file "dataset.py" where we return the image tensor (data) and label

        # put data and labels on device
        X, imgs, labels = X.to(device), imgs.to(device), labels.to(device)

        # forward pass
        prediction = model(X, imgs)
...
def validate(cfg, dataLoader, model):
    ...
    with torch.no_grad():               # don't calculate intermediate gradient steps: we don't need them, so this saves memory and is faster
        for idx, (X, imgs, labels) in enumerate(dataLoader):
